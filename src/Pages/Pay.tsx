...
import UseDetailUser from "../Hook/useDetailUser.tsx";
import { useForm } from "react-hook-form";
import { z } from "zod";
import { zodResolver } from "@hookform/resolvers/zod";
import { useMutation, useQueryClient } from "react-query";
import { addOrder, deleteAllCart } from "../Apis/Api.tsx";
const Pay = () => {
  ...
  const { mutate } = useMutation({
    mutationFn: (data: object) => addOrder(data),
    onSuccess: (data: any) => {
      mutateDelete();
  ...
  useEffect(() => {
    if (data) {
      const product = data?.data.map((item: cart) => {
        return {
