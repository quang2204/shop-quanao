...
import { deleteCart, getVouchers, updateCart } from "../Apis/Api.tsx";
import { useEffect, useState } from "react";
import cart from "../Interface/cart.tsx";
import voucher from "../Interface/Voucher.tsx";
...
const ShopingCart = () => {
  ...
  const { mutate, isLoading: isLoadingCart } = useMutation({
    mutationFn: ({ data, id }: { data: object; id: number }) =>
      updateCart(data, id),
    onSuccess: () => {
  ...
  const increaseNumber = (id: string) => {
    const check = counts?.find((item: any) => item.id === id);
    if (check) {
  ...
  const numberDirectly = (input: string, id: string) => {
    const check = counts.find((item: any) => item.id === id);
    if (check) {
...
const Voucher = ({
  ...
  setVoucher: React.Dispatch<React.SetStateAction<number>>;
  setVoucherId: React.Dispatch<React.SetStateAction<number>>;
  voucherid: number;
}) => {
  const [check, setCheck] = useState<boolean>(false);
  const [beforeVoucher, setBeforeVoucher] = useState<number>(0);
  ...
  const handleVoucherId = (discount: any, id: number) => {
                ...
                <div className="d-flex align-items-center">
                  ...
                  <div className="conten p-l-20 fs-20">
                    ...
                    <p className="text-muted text-lg">
                      Hạn sử dụng :
                      {new Date(item.expire).toLocaleDateString("vi-VN")}
                    </p>
                  </div>
                </div>
                <>
